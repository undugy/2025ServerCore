...
template<class T>
OverlappedEx* MakeSendBuffer(T& pkt, uint32_t pktId)
{
	...
	PacketHeader* header = reinterpret_cast<PacketHeader*>(over->IOCP_buf);
...
template<class T>
void SendPacket(int cl, uint32_t pktId, T& packet)
{

	OverlappedEx* over =  MakeSendBuffer<T>(packet, pktId);
...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	switch (packet[1]) {
	case EPacketProtocol::SC_ChatResponse: {
		SCChatResponse pkt;
		if (pkt.ParseFromArray(packet + sizeof(PacketHeader), header->size - sizeof(PacketHeader)) == true)
		{
			if (pkt.sessionid() < MAX_CLIENTS) {
				int my_id = client_map[pkt.sessionid()];
				if (-1 != my_id) {
	...
	case EPacketProtocol::SC_GetIDResponse:
	{
		SCGetIDResponse pkt;
...
void Adjust_Number_Of_Client()
{
	...
	CSGetIDRequest packet;
	SendPacket(num_connections, EPacketProtocol::CS_GetIDRequest, packet);
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			CSChatRequest packet;
			packet.set_sessionid(g_clients[i].id);
			packet.set_msg("Testclient"+std::to_string(client_map[i]));
			SendPacket(i,EPacketProtocol::CS_ChatRequest, packet);
		}
	}
}
...
void GetMsgCloud(std::vector<std::string>&msgVec)
{
	...
	for (int i = 0; i < num_connections; ++i)
		if (true == g_clients[i].connected) {
			msgVec.push_back(std::move(g_clients[i].msg));
